...
class MessagesStore {
	...
	constructor() {
		if (getIsCustomizing()) {
			...
			this.items.push(new Message("День добрый, сейчас найдем решение", new Date(), 'server', {name: 'Майкл', surname: 'Жордан'}))
		}
	}

	private getMessagesFromStorage(): Message[] {
		return JSON.parse(localStorage.getItem(`${prefix}-messages`) || '[]').map((item: Message) => {
			const sender = {name: item.name, surname: item.surname};
			return new Message(item.content, new Date(item.timeStamp), item.side, sender, item.fileKey);
		});
	}
	...
	setMessage(mes: Message) {
		...
		localStorage.setItem(`${prefix}-messages`, JSON.stringify(this.itemsInLocalStorage));
	}
