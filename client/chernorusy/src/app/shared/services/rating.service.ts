...
export class RatingService {
  ...
  public getMany$(managersIds: string[]) {
    const haveNoRating = managersIds.filter(
      (id) => !(id in this.allRatings$.value)
    );
    ...
    return this.getManyForce$(haveNoRating).pipe(
      switchMap(() =>
        this.allRatings$.pipe(
          map((allRatings) => pick(allRatings, managersIds))
        )
      )
    );
  ...
  public getManyForce$(managersIds: string[]): Observable<AllRatings> {
    return this.httpS
      .post<RatingGetResponse[]>('/Rating/Statistics', { managersIds })
      .pipe(
        catchError(() => []),
        tap((ratings) => {
          const curRatings = this.allRatings$.value;
          ratings.forEach((rating) => {
            curRatings[rating.managerId] = {
  ...
  public getID$(id: string): Observable<{ count: number; avg: number }> {
    ...
    return this.getManyForce$([id]).pipe(switchMap(() => this.getID$(id)));
  }
}
...
export type AllRatings = {
  [managerId: string | number]: { count: number; avg: number };
};
